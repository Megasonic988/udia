defmodule Udia.Web.Api.CommentController do
  ...
  def index(conn, _params) do
    ...
    render(conn, Udia.Web.CommentView,"index.json", comments: comments)
  end
  ...
  def create(conn, %{"comment" => comment_params}) do
    with {:ok, %Comment{} = comment} <- Logs.create_comment(comment_params) do
      ...
      |> render(Udia.Web.CommentView, "show.json", comment: comment)
    end
  end

  def show(conn, %{"id" => id}) do
    comment = Logs.get_comment!(id)
    render(conn, Udia.Web.CommentView, "show.json", comment: comment)
  end
